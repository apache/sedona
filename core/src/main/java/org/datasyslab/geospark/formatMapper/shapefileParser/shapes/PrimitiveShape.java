...
public class PrimitiveShape implements Serializable {
    ...
    public Geometry getShape(GeometryFactory geometryFactory) throws IOException, TypeUnknownException {
        ...
        if(parser == null) {
            throw new TypeUnknownException(shapeType.getId());
        }

        ByteBuffer shapeBuffer = ByteBuffer.wrap(primitiveRecord).order(ByteOrder.LITTLE_ENDIAN);
        Geometry shape = parser.parseShape(shapeBuffer);
        if(attributes != null){
