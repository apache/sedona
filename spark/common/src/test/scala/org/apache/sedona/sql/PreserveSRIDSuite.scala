...
class PreserveSRIDSuite extends TestBaseScala with TableDrivenPropertyChecks {
  ...
  describe("Preserve SRID") {
    val testCases = Table(
      ...
      ("ST_GeometryN(geom2, 3)", 1000),
      ("ST_InteriorRingN(geom4, 1)", 1000),
      ("ST_Dump(geom2)", 1000),
      ...
      ("ST_Polygon(ST_InteriorRingN(geom4, 1), 2000)", 2000),
      ("ST_Polygonize(geom5)", 1000),
      ("ST_Project(geom5, 12, 12, true)", 1000),
      ("ST_MakePolygon(ST_ExteriorRing(geom4), ARRAY(ST_InteriorRingN(geom4, 1)))", 1000),
