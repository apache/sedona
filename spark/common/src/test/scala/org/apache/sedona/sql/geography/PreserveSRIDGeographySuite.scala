 ...
 * ST_GeomToGeography) are tested directly. Scalar/predicate functions on Geography inputs are
 * wrapped in an identity passthrough — `IF(<scalar/pred>, geog1, geog1)` — so the surrounding
 * expression returns a Geography whose SRID can be asserted; failure of such a row signals either
 * an evaluation failure on the wrapped function or SRID being dropped somewhere in the chain.
 */
