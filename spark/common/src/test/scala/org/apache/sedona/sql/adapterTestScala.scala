...
class adapterTestScala extends TestBaseScala with GivenWhenThen {
  ...
  describe("Sedona-SQL Scala Adapter Test") {
    ...
    it("Read CSV point at a different column id into a SpatialRDD") {
      ...
      assert(
        newDf.schema.toList.map(f => f.name).mkString("\t").equals("geometry\t123\t456\t789"))
    }
    ...
    it("Read shapefileWithMissing -> DataFrame") {
      var spatialRDD = ShapefileReader.readToGeometryRDD(
        sparkSession.sparkContext,
        shapefileWithMissingsTrailingInputLocation)
      spatialRDD.analyze()
      var df = Adapter.toDf(spatialRDD, sparkSession)
      assert(df.count() == 3)
    }
    ...
    it("Read GeoJSON to DataFrame") {
      var spatialRDD = new PolygonRDD(
        sparkSession.sparkContext,
        geojsonInputLocation,
      ...
      var df = Adapter.toDf(
        spatialRDD,
        sparkSession
      ) // .withColumn("geometry", callUDF("ST_GeomFromWKT", col("geometry")))
    ...
    it("Convert spatial join result to DataFrame") {
      ...
      val joinResultDf2 =
        Adapter.toDf(joinResultPairRDD, polygonRDD.fieldNames.asScala.toSeq, List(), sparkSession)
    ...
    ignore("load HDF data from RDD to a DataFrame") {
      ...
      val HDFDataVariableList: Array[String] =
        Array("LST", "QC", "Error_LST", "Emis_31", "Emis_32")
      val earthdataHDFPoint = new EarthdataHDFPointMapper(
        ...
        urlPrefix)
      val spatialRDD =
        new PointRDD(sparkSession.sparkContext, InputLocation, numPartitions, earthdataHDFPoint)
      import scala.jdk.CollectionConverters._
    ...
    it("can convert spatial RDD with user data to a valid Dataframe") {
      val srcDF = sparkSession.sql(
        "select ST_PointFromText('40.7128,-74.0060', ',') as geom, \"attr1\" as attr1, \"attr2\" as attr2")
    ...
    it("can convert spatial RDD to DataFrame with user-supplied schema") {
      ...
      assert(df.schema == schema, s"Expected schema\n$schema\nbut got\n${df.schema}!")
    }
    ...
    /**
     * If providing fieldNames instead of a schema, it should return the same result as if we do
     * provide a schema but declare all non-geom fields to be StringType
     */
    it("is consistent with toDf without schema vs with all StringType fields") {
      ...
      val resultWithoutSchema = Adapter.toDf(
        joinResultPairRDD,
        Seq("exampletext", "exampledouble", "exampleint"),
        Seq("userdata"),
        sparkSession)
    ...
    it("can convert spatial pair RDD with user data to a valid Dataframe") {
      ...
      var leftDF = sparkSession.sql(
        "select ST_GeomFromWKT(inputtable._c0) as leftGeom, \"attr1\" as attr1, \"attr2\" as attr2 from inputtable")
      val rightDF = sparkSession.sql(
        "select ST_PointFromText('40.7128,-74.0060', ',') as rightGeom, \"attr3\" as attr3, \"attr4\" as attr4")
      ...
      val pairDF =
        Adapter.toDf(pairRDD, Seq("attr1", "attr2"), Seq("attr3", "attr4"), sparkSession)
