...
class TestStGeometryFromGeoHash
  ...
  describe("should create geometries based on hash using ST_GeomFromGeoHash function") {
    it("should create polygon geometry based on geohash") {
      ...
      val geoHashCollection = Seq(
        ...
        (
          "s12fd",
          "POLYGON ((1.142578125 7.470703125, 1.142578125 7.5146484375, 1.1865234375 7.5146484375, 1.1865234375 7.470703125, 1.142578125 7.470703125))"),
        (
          "2131s12fd",
      ...
      val result = geoHashDf
        .selectExpr("ST_AsText(ST_GeomFromGeoHash(geohash)) AS wkt")
    ...
    it("should create geometry based on geohash and precision") {
      ...
      When("creating Geometry from geohash")
      val result = geoHashDf
        .selectExpr("ST_AsText(ST_GeomFromGeoHash(geohash, 2)) AS wkt")
        .where("wkt is not null")
        .as[String]
        .collect()
        .toList
    ...
    it("should return null geometries when geohash or given precision value is invalid") {
      ...
      val invalidGeoHashDf = Seq(("aidft35")).toDF("geohash")

      val validGeoHash = Seq(("sd")).toDF("geohash")
    ...
    it("should return null for null values") {
      ...
      val geometryDf = dataFrameWithNulls
        .selectExpr("ST_GeomFromGeoHash(geohash) AS geom")
        .filter("geom IS NOT NULL")
