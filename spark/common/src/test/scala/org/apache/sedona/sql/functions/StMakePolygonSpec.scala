  ...
  describe("should pass ST_MAkePolygon") {
    ...
    it("should correctly create polygon based on given linestring and holes") {
      ...
      val geometryDataFrame = Seq(
        (
          "LINESTRING(0 5, 1 7, 2 9, 2 5, 5 7, 4 6, 3 2, 1 3, 0 5)",
          Seq("LINESTRING(2 3, 1 4, 2 4, 2 3)", "LINESTRING(2 4, 3 5, 3 4, 2 4)")),
        ...
        .map { case (geom, geometries) =>
          Tuple2(wktReader.read(geom), geometries.map(wktReader.read))
        }
        .toDF("geom", "holes")
    ...
    it(
      "should return null values when calling ST_MakePolygon on given geometries which are not valid") {
      Given("line strings which can not be valid polygon")
      val geometryDataFrame = Seq("LINESTRING(2 1,3 4,4 6)", "POINT(21 52)")
        .map(geom => Tuple1(wktReader.read(geom)))
        .toDF("geom")
      ...
      val result = geometryDfWithPolygon
        .select("poly")
