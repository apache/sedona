...
class CoverageValidationFunctions extends TestBaseScala with Matchers {

  private val functionName = "ST_CoverageInvalidEdgesForTarget"

  describe("coverage-validation expression") {

    it("registers the public function with a default gap width") {
      sparkSession.catalog.functionExists(functionName) should be(true)
      sparkSession.catalog
        .functionExists("__sedona_internal_coverage_invalid_edges_for_target") should be(false)

      val invalidEdges = coverageSource
        .selectExpr(s"$functionName(target, matching_adjacent) AS invalid_edges")
        .first()
