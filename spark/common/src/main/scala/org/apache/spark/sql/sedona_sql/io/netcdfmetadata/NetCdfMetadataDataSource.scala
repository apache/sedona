...
/**
 * A read-only Spark SQL data source that extracts NetCDF file metadata (dimensions, variables,
 * attributes, spatial extent) without loading data arrays into memory. Only the 1-D coordinate
 * variable arrays are read when the spatial extent is requested.
 */
class NetCdfMetadataDataSource
