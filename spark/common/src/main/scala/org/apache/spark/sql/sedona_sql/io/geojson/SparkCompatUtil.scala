...
object SparkCompatUtil {
  ...
  def findNestedField(schema: StructType, path: Array[String], resolver: Resolver): Option[StructField] = {
    path match {
      case Array(part) =>
  ...
  def constructTimestampFormatter(
    ...
    val timestampFormatInWriteTerm = ru.typeOf[JSONOptions].decl(ru.TermName("timestampFormatInWrite"))
  ...
  def constructDateFormatter(
    ...
    val dateFormatterClass = mirror.staticClass("org.apache.spark.sql.catalyst.util.DateFormatter$")
    ...
    val applyMethods = dateFormatterClass.toType.members.filter(_.name.decodedName.toString == "apply")
  ...
  def constructJacksonParser(
    ...
    val mirror = ru.runtimeMirror(getClass.getClassLoader)
    val jacksonParserClass = mirror.staticClass("org.apache.spark.sql.catalyst.json.JacksonParser")
