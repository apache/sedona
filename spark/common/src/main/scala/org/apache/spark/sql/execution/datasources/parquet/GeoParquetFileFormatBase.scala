...
/**
 * Trait of GeoParquetFileFormat defines a method for spatial predicate push down. The actual
 * implementations of GeoParquetFileFormat are in the sql/spark-3.x submodule for specific Spark
 * minor versions.
 */
trait GeoParquetFileFormatBase extends FileFormat with DataSourceRegister {
   ...
   * @param spatialFilter
   *   spatial filter pushed down to GeoParquetFileFormat
   * @return
   *   a new GeoParquetFileFormat object
   */
