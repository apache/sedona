...
object AggregateWithinPartitons {
   ...
   * three aggregates, sum, count, avg. If the aggregate func is count, this function doesn't
   * require a value column. If the aggregate func is sum and avg, it will require a value column.
   * They are same as the regular aggregation SQL
   *
  ...
  def apply(
    ...
    if (aggFunc.equalsIgnoreCase("count"))
      formattedDf = dataFrame
        .select(keyCol, Conf.PrimaryPID, Conf.SecondaryPID)
        .withColumn(valueCol, lit(0.0))
      ...
      aggFunc match {
        ...
        case _ =>
          throw new IllegalArgumentException(
            "[Sedona-Viz][SQL] Unsupported aggregate func type. Only sum, count, avg are supported.")
      }
      result.iterator
    })
