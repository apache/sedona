...
public class SpatialRDD<T extends Geometry> implements Serializable {
    ...
    public void calc_partitioner(GridType gridType, int numPartitions) throws Exception {
        ...
        switch (gridType) {
            ...
            case ImprovedQT: {
                ImprovedQuadtreePartitioning quadtreePartitioning = new ImprovedQuadtreePartitioning(this, numPartitions);
                ImprovedQuadTree tree = quadtreePartitioning.getPartitionTree();
                partitioner = new ImprovedQuadTreePartitioner(tree);
                break;
            }
    ...
    public void calc_partitioner(GridType gridType, int numPartitions, boolean needSample) throws Exception {
        if (needSample) {
        ...
        switch (gridType) {
            ...
            case QUADTREE: {
                QuadtreePartitioning quadtreePartitioning =
                        new QuadtreePartitioning(data, paddedBoundary, numPartitions);
                StandardQuadTree tree = quadtreePartitioning.getPartitionTree();
                partitioner = new QuadTreePartitioner(tree);
                break;
            }
            case KDBTREE: {
                final KDB tree = new KDB(data.size() / numPartitions, numPartitions, paddedBoundary);
                for (final Envelope sample : data) {
                    tree.insert(sample);
                }
                tree.assignLeafIds();
                partitioner = new KDBTreePartitioner(tree);
            ...
            case STRTREE: {
                RtreePartitioning rtreePartitioning = new RtreePartitioning(data, numPartitions);
                partitioner = new FlatGridPartitioner(rtreePartitioning.getGrids());
                break;
            }
            case Voronoi: {
                VoronoiPartitioning voronoiPartitioning = new VoronoiPartitioning(data, numPartitions);
                partitioner = new FlatGridPartitioner(voronoiPartitioning.getGrids());
                break;
            }
            case Hilbert: {
                HilbertPartitioning hilbertPartitioning = new HilbertPartitioning(data, paddedBoundary, numPartitions);
    ...
    public void spatialPartitioning(GridType gridType, int numPartitions, boolean needSample) throws Exception {
        calc_partitioner(gridType, numPartitions, needSample);
        this.spatialPartitionedRDD = partition(partitioner);
    }
