...
import org.apache.sedona.sql.datasources.geopackage.model.{GeoPackageField, GeoPackageOptions, GeoPackageReadOptions, PartitionOptions, TableType}
...
case class GeoPackagePartitionReaderFactory(
  ...
  override def createReader(partition: InputPartition): PartitionReader[InternalRow] = {
    ...
    var cursor: java.sql.ResultSet = null
    try {
      ...
      val rs = if (loadOptions.includeGeometryType) {
        GeoPackageConnectionManager.getMetadataCursor(tempFile.getAbsolutePath)
      } else {
        GeoPackageConnectionManager.getTableCursor(
      ...
      val schema =
        GeoPackageConnectionManager.getSchema(tempFile.getPath, loadOptions.tableName) ++
          (if (loadOptions.includeGeometryType)
             Seq(GeoPackageField("geometry_type", "TEXT", true))
           else Seq.empty)
        ...
        case TILES =>
          Some(
            GeoPackageConnectionManager
              .findTilesMetadata(tempFile.getPath, loadOptions.tableName))
        case _ => None
      }
      ...
      val baseReader = GeoPackagePartitionReader(
        ...
        options = GeoPackageReadOptions(
          ...
          includeGeometryType = loadOptions.includeGeometryType),
