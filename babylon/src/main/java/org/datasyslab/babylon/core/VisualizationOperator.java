...
public abstract class VisualizationOperator implements Serializable{
    ...
    private boolean spatialPartitioningWithDuplicates() throws Exception
    {
        this.distributedRasterCountMatrix = this.distributedRasterCountMatrix.flatMapToPair(new PairFlatMapFunction<Tuple2<Pixel, Double>, Pixel, Double>() {
            @Override
            public List<Tuple2<Pixel, Double>> call(Tuple2<Pixel, Double> pixelDoubleTuple2) throws Exception {
                VisualizationPartitioner vizPartitioner = new VisualizationPartitioner(resolutionX,resolutionY,partitionX,partitionY);
                return vizPartitioner.assignPartitionIDs(pixelDoubleTuple2, photoFilterRadius);
            }
        });
    ...
    protected JavaPairRDD<Pixel, Double> ApplyPhotoFilter(JavaSparkContext sparkContext) throws Exception
    {
        ...
        if(this.parallelPhotoFilter)
        {
            ...
            this.distributedRasterCountMatrix = this.distributedRasterCountMatrix.mapPartitionsToPair(new PairFlatMapFunction<Iterator<Tuple2<Pixel,Double>>,Pixel,Double>()
            {
                ...
                @Override
                public List<Tuple2<Pixel, Double>> call(Iterator<Tuple2<Pixel, Double>> currentPartition) throws Exception  {
        ...
        else
        {
            ...
            this.distributedRasterCountMatrix = this.distributedRasterCountMatrix.flatMapToPair(new PairFlatMapFunction<Tuple2<Pixel, Double>, Pixel, Double>() {
                @Override
                public List<Tuple2<Pixel, Double>> call(Tuple2<Pixel, Double> pixelCount) throws Exception {
    ...
    protected boolean Colorize()
    {
        ...
        else
        {
            ...
            final Double maxWeight = this.maxPixelCount;
            logger.info("[Babylon][Colorize]maxCount is "+maxWeight);
            final Double minWeight = 0.0;
            ...
            //logger.debug("[Babylon][Colorize]output count "+this.distributedRasterColorMatrix.count());
        }
    ...
    protected boolean RenderImage(JavaSparkContext sparkContext) throws Exception
    {
        ...
        else if(this.parallelRenderImage==true && this.generateVectorImage==false)
        {
            ...
            this.distributedRasterImage = this.distributedRasterColorMatrix.mapPartitionsToPair(
                    ...
                    {
                        @Override
                        public List<Tuple2<Integer, ImageSerializableWrapper>> call(Iterator<Tuple2<Pixel, Integer>> currentPartition)
                                throws Exception {
                            ...
                            logger.info("[Babylon][Render]add a image partition into result set "+pixelColor._1().getCurrentPartitionId());
            ...
            //logger.debug("[Babylon][Render]output count "+this.distributedRasterImage.count());
        }
    ...
    protected JavaPairRDD<Pixel, Double> Rasterize(JavaSparkContext sparkContext,
        ...
        if(generateVectorImage)
        {
            this.distributedVectorObjects = rawSpatialRDD.flatMapToPair(new PairFlatMapFunction<Object,Object, Double>(){
                @Override
                public List<Tuple2<Object,Double>> call(Object spatialObject) throws Exception {
            ...
            {
                @Override
                public List<Tuple2<Pixel, Double>> call(Object spatialObject) throws Exception {

                    if(spatialObject instanceof Point)
                    {
                        return RasterizationUtils.FindPixelCoordinates(resolutionX,resolutionY,datasetBoundary,(Point)spatialObject,colorizeOption,reverseSpatialCoordinate);
                    }
                    else if(spatialObject instanceof Polygon)
                    {
		...
		*/
            ...
            //logger.debug("[Babylon][Rasterize]output count "+this.distributedRasterCountMatrix.count());
        }
    ...
    protected JavaPairRDD<Pixel, Double> Rasterize(JavaSparkContext sparkContext,
        ...
        if(generateVectorImage)
        {
            ...
            this.distributedVectorObjects = spatialPairRDD.flatMapToPair(new PairFlatMapFunction<Tuple2<Polygon,Long>,Object,Double>(){
                @Override
                public List<Tuple2<Object, Double>> call(Tuple2<Polygon, Long> spatialObject) throws Exception {
                    GeometryFactory geometryFactory = new GeometryFactory();
                    List<Tuple2<Object,Double>> result = new ArrayList<Tuple2<Object,Double>>();
        ...
        else
        {
            ...
            {
                @Override
                public List<Tuple2<Pixel, Double>> call(Tuple2<Polygon,Long> spatialObject) throws Exception {
                    return RasterizationUtils.FindPixelCoordinates(resolutionX,resolutionY,datasetBoundary,(Polygon)spatialObject._1(),reverseSpatialCoordinate,new Double(spatialObject._2()));
                }
            });
