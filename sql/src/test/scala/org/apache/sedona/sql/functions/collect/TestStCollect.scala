  ...
  describe("st collect workflow"){
    it("should return null when passed geometry is also null"){
      ...
      withCollectedGeometries
        ...
        .distinct().as[String]
        .collect.head shouldBe "GEOMETRYCOLLECTION EMPTY"
    ...
    it("should not fail if any element in given array is empty"){
      ...
      expectedGeoms should contain theSameElementsAs Seq(
        "GEOMETRYCOLLECTION EMPTY", "MULTILINESTRING ((0 0, 1 1, 2 2), (5 7, 4 3, 1 1, 0 0))",
    ...
    it("should return multi geometry for an array of geometries"){
      ...
      val emptyGeometryDataFrame = Seq(
        ...
        (2, Seq(
          "LINESTRING(1 2, 3 4)", "LINESTRING(3 4, 4 5)"
        ).map(wktReader.read)),
        (3, Seq(
          "POINT(1 2)", "POINT(-2 3)"
        ).map(wktReader.read))
    ...
    it("should return multi geometry for a list of geometries"){
      ...
      val geometryDf = Seq(
        (1, "POINT(21 52)", "POINT(43 56)", "POINT(12 34)", "POINT(34 67)"),
      ).map{
        case (id, geomFirst, geomSecond, geomThird, geomFourth) =>
          (
            id, wktReader.read(geomFirst), wktReader.read(geomSecond), wktReader.read(geomThird),
      ...
      stCollectResult
        ...
        Seq("MULTIPOINT ((21 52), (43 56), (12 34), (34 67))")
      )
    ...
    it("should filter out null values if exists in any of passed column"){
      ...
      val geometryDf = Seq(
        (1, null, "POINT(43 56)", null, "POINT(34 67)"),
      ).map{
        case (id, geomFirst, geomSecond, geomThird, geomFourth) =>
          (
            id, geomFirst, wktReader.read(geomSecond), geomThird,
            wktReader.read(geomFourth)
          )
      }.toDF("id", "geomFirst", "geomSecond", "geomThird", "geomFourth")
    ...
    it("should return multitype when only one column is passed"){
      ...
      val geometryDf = Seq(
        ...
        case (id, geom) =>
          (
            id, wktReader.read(geom)
          )
      }.toDF("id", "geom")

      When("running on st collect on one geometry column")
      val geometryDfWithCollected = geometryDf.withColumn(
        "collected", expr("ST_Collect(geom)")
      ).selectExpr("ST_AsText(collected)").as[String]
