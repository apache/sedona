...
abstract class InferredNaryExpression[A1: InferrableType, A2: InferrableType, A3: InferrableType, A4: InferrableType,
                                        A5: InferrableType, A6: InferrableType, A7: InferrableType, A8: InferrableType, A9: InferrableType,
                                        A10: InferrableType, A11: InferrableType, A12: InferrableType, A13: InferrableType, R: InferrableType]
                                          (f: (A1, A2, A3, A4, A5, A6, A7, A8, A9, A10, A11, A12, A13) => R)
                                          (implicit val a1Tag: TypeTag[A1], implicit val a2Tag: TypeTag[A2], implicit val a3Tag: TypeTag[A3], implicit val a4Tag: TypeTag[A4],
  ...
  override def inputTypes: Seq[AbstractDataType] = Seq(inferSparkType[A1], inferSparkType[A2], inferSparkType[A3], inferSparkType[A4], inferSparkType[A5],
                                                    inferSparkType[A6], inferSparkType[A7], inferSparkType[A8], inferSparkType[A9], inferSparkType[A10],
                                                    inferSparkType[A11], inferSparkType[A12], inferSparkType[A13])

  override def nullable: Boolean = true

  override def dataType = inferSparkType[R]

  lazy val extractFirst = buildExtractor[A1](inputExpressions.head)
  lazy val extractSecond = buildExtractor[A2](inputExpressions(1))
  lazy val extractThird = buildExtractor[A3](inputExpressions(2))
  lazy val extractFourth = buildExtractor[A4](inputExpressions(3))
  lazy val extractFifth = buildExtractor[A5](inputExpressions(4))
  lazy val extractSixth = buildExtractor[A6](inputExpressions(5))
  lazy val extractSeventh = buildExtractor[A7](inputExpressions(6))
  lazy val extractEighth = buildExtractor[A8](inputExpressions(7))
  lazy val extractNinth = buildExtractor[A9](inputExpressions(8))
  lazy val extractTenth = buildExtractor[A10](inputExpressions(9))
  lazy val extractEleventh = buildExtractor[A11](inputExpressions(10))
  lazy val extractTwelfth = buildExtractor[A12](inputExpressions(11))
  lazy val extractThirteenth = buildExtractor[A13](inputExpressions(12))
