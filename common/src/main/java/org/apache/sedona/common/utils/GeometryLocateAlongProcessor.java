...
public class GeometryLocateAlongProcessor {
    ...
    public GeometryLocateAlongProcessor() {
        geometryFunctions.put(Point.class, (geometry, params) -> locateAlongPoint((Point) geometry, params[0], params[1]));
        geometryFunctions.put(MultiPoint.class, (geometry, params) -> locateAlongMultiPoint((MultiPoint) geometry, params[0], params[1]));
        geometryFunctions.put(LineString.class, (geometry, params) -> locateAlongLineString((LineString) geometry, params[0], params[1]));
        geometryFunctions.put(MultiLineString.class, (geometry, params) -> locateAlongMultiLineString((MultiLineString) geometry, params[0], params[1]));
    }
    ...
    private Geometry locateAlongMultiPoint(MultiPoint multiPoint, double measure, double offset) {
        ...
        return multiPoint.getFactory().createMultiPoint(Arrays.stream(points).filter(Objects::nonNull).toArray(Point[]::new));
    }
    ...
    private Geometry locateAlongLineString(LineString lineString, double measure, double offset) {
        ...
        return lineString.getFactory().createMultiPointFromCoords(Arrays.stream(coordinateList.toCoordinateArray()).filter(Objects::nonNull).toArray(Coordinate[]::new));
    }
    ...
    private Geometry locateAlongMultiLineString(MultiLineString multiLineString, double measure, double offset) {
        ...
        for (int i = 0; i < multiLineString.getNumGeometries(); i++) {
            MultiPoint mPoint = (MultiPoint) locateAlongLineString((LineString) multiLineString.getGeometryN(i), measure, offset);
