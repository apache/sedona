...
def _ensure_geometry(
    data, crs: Any | None = None
) -> sgpd.GeoSeries:  #  | GeometryArray
    """
...
class GeoDataFrame(GeoFrame, pspd.DataFrame):
    ...
    def __getitem__(self, key: Any) -> Any:
        ...
        if isinstance(key, str):
            ...
            # Here we are getting a ps.Series with the same underlying anchor (ps.Dataframe) here.
            # This is important so we don't run into errors when trying to perform operations on different dataframes
            ...
            if field and (
                ...
                # pd_series = pandas_df[column_name]
                # Translating from spark to pandas changes the underlying data from wkb to arrays of ints
                # if field.dataType.typeName() == "binary":
                #     pd_series = pd_series.apply(bytes)

                # TODO: .apply(bytes)
                # print(ps_series.apply(bytes))  # didn't work
                geoseries = sgpd.GeoSeries(ps_series)
                return geoseries
            else:
        ...
        # Handle row selection via slice or boolean indexing
            ...
            # TODO: modify to use same underlying anchor
    ...
    def set_geometry(
        ...
        # TODO: this currently fails because GeoSeries.set_crs returns a Series with a different anchor
        # level.crs = crs
