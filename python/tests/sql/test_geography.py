...
class TestGeographyConstructorsDataFrameAPI(TestBase):
    ...
    def test_st_geog_from_ewkb(self):
        ...
        ewkb_bytes = bytes(
            [
                1, 2, 0, 0, 32, 230, 16, 0, 0, 2, 0, 0, 0,
                0, 0, 0, 0, 132, 214, 0, 192, 0, 0, 0, 0, 128, 181, 214, 191,
                0, 0, 0, 96, 225, 239, 247, 191, 0, 0, 0, 128, 7, 93, 229, 191,
            ]
        )
...
class TestGeographyFunctionsDataFrameAPI(TestBase):
    ...
    def test_st_distance(self):
        df = self.spark.range(1).select(
            stf.ST_Distance(
                self._geog("POINT (0 0)"), self._geog("POINT (1 1)")
            ).alias("d")
        )
        d = df.first()[0]
