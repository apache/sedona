...
def create_sample_polygons_df(
    spark: SparkSession, number_of_polygons: int
) -> DataFrame:
    return resource_file_to_dataframe(spark, "sample_polygons").limit(
        number_of_polygons
    )
...
def create_simple_polygons_df(
    spark: SparkSession, number_of_polygons: int
) -> DataFrame:
    return resource_file_to_dataframe(spark, "simple_polygons").limit(
        number_of_polygons
    )
...
def resource_file_to_dataframe(spark: SparkSession, file_path: str) -> DataFrame:
    ...
    schema = StructType([StructField("geom", GeometryType(), True)])
