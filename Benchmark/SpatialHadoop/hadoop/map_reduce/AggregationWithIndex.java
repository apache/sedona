...
public class AggregationWithIndex {
	...
	AggregationWithIndex(String hdfs, String inputRec1, String inputRec2,
				String outputRec) {
		this.hdfspath = hdfs;
	...
	public void run() throws IOException {
		...
		//begin phase 1 
		JobConf conf0 = new JobConf(AggregationWithIndex.class);
		conf0.setJobName("ParseInput");
		...
		JobConf conf = new JobConf(AggregationWithIndex.class);
		conf.setJobName("ParseInput");
		...
		try {
			Process p = Runtime.getRuntime().exec("shadoop index "+ inputRec1 + ".parse" + " " + inputRec1 + ".index" + " " + "sindex:rtree" + " " +"shape:wkt" + " " + "-overwrite");
		...
		try {
			Process p = Runtime.getRuntime().exec("shadoop index "+ inputRec2 + ".parse" + " " + inputRec2 + ".index" + " " + "sindex:rtree" + " " +"shape:wkt" + " " + "-overwrite");
			InputStream stderr = p.getErrorStream();
			InputStream stdout = p.getInputStream();
            InputStreamReader isr = new InputStreamReader(stderr);
            InputStreamReader iso = new InputStreamReader(stdout);
		...
		try {
			Process p = Runtime.getRuntime().exec("shadoop sjmr "+ inputRec1 + ".index" + " " + inputRec2 + ".index" + " " + "aggtemp" + " shape:wkt " + "-overwrite");
			InputStream stderr = p.getErrorStream();
			InputStream stdout = p.getInputStream();
            InputStreamReader isr = new InputStreamReader(stderr);
            InputStreamReader iso = new InputStreamReader(stdout);
		...
		// Phase 3: aggregate result from phase 2.
		JobConf conf1 = new JobConf(AggregationWithIndex.class);
	...
	public static void main(String args[]) throws IOException {
    	/*
	   	 ...
	   	 args[1] is the name of input file1
	   	 args[2] is the name of input file2
	   	 args[3] is the name of output file. 
	   	 */
	   	System.out.println("This is aggregation application based on spatialHadoop r-tree index");
		...
		AggregationWithIndex agg = new AggregationWithIndex(args[0], args[1], args[2], args[3]);
		agg.run();
	}
}
