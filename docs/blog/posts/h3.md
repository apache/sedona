...
![Image showing H3 nested grid cells over San Francisco](../../../docs/image/blog/h3/image5.png)
...
Tesselation, also known as tiling, is how tiles can be arranged to cover a plane, without any gaps.
...
There are however some significant tradeoffs when using the H3 index for spatial data in terms of accuracy, data preservation, data transformation and processing, and more.
...
![H3 cells overlapping a bounding box in London](../../../docs/image/blog/h3/image2.png)
[https://stackoverflow.com/questions/75860703/h3-how-to-get-the-h3-index-of-all-cells-that-are-at-least-partially-inside-a-b](https://stackoverflow.com/questions/75860703/h3-how-to-get-the-h3-index-of-all-cells-that-are-at-least-partially-inside-a-b)
...
empire\_state\_building \= Point(-73.985428, 40.748817)  
freedom\_tower \= Point(-74.013379, 40.712743)  
willis\_tower \= Point(-87.635918, 41.878876)

df \= sedona.createDataFrame(\[  
    (empire\_state\_building, freedom\_tower),  
    (empire\_state\_building, willis\_tower),  
\], \["start", "end"\])
...
res \= df.withColumn(  
    ...
    ST\_DistanceSphere(col("start"), col("end"))  
).withColumn(  
    "start\_cellid",   
    ST\_H3CellIDs(col("start"), 6, True)  
).withColumn(  
    "end\_cellid",   
    ST\_H3CellIDs(col("end"), 6, True)  
).withColumn(  
    "h3\_cell\_distance",   
    ST\_H3CellDistance(col("start\_cellid")\[0\], col("end\_cellid")\[0\])  
)

There are a few important points to note in this code snippet:

* You must provide a resolution when instantiating the ST\_H3CellIDs.  For example, ST\_H3CellIDs(col("start"), 6, True) uses a resolution of 6\.  
* The ST\_H3CellDistance returns an integer representing the number of steps (hexagons) between the two H3 cells on the H3 grid.

Let’s take a look at the resulting DataFrame:

res.select("st\_distance\_sphere", "h3\_cell\_distance").show()

\+------------------+----------------+  
|st\_distance\_sphere|h3\_cell\_distance|  
\+------------------+----------------+  
|4651.5708314048225|               1|  
|1145748.4514602514|             180|  
\+------------------+----------------+

The ST\_DistanceSphere function returns the distance between the two points in meters and the ST\_H3CellDistance function returns the number of H3 cells between two H3 cells, for a given resolution.
...
- ST\_H3CellDistance: Measure the cell distance between two cells (not the real world distance)  
- ST\_H3CellIDs: Creates an array of H3 cell ids to cover polygons or linestrings  
- ST\_H3KRing: Produces the "filled-in disk" of cells which are at most grid distance k from the origin cell.  
- ST\_H3ToGeom: Turns an H3 cell into a hexagonal geometry
...
![Single FEMA Flood Plain polygon near Houston Texas](../../../docs/image/blog/h3/image6.png)
...
![H3 cells at level 8 overlapping a FEMA Flood plain polygon near Houston Texas](../../../docs/image/blog/h3/image3.png)  
...
![Animated GIF showing the differences between the original FEMA flood polygon, H3 cells at level 8 and level 12, and buildings that fall in each of the different areas](../../../docs/image/blog/h3/image4.gif)  
...
Let’s test that with the level 8 H3 cells. Note that you have to first transform the cells to geometries, then aggregate them into a single polygon using ST\_Union\_Agg to avoid counting buildings more than one time. 
...
Closer but still over-counting by 45 buildings compared to the true polygon. With Apache Sedona you don’t need to add any extra steps to process data at scale and you don’t need to compromise accuracy. Here is a final map of the three compared layers:
...
![Data skew using different spatial partitionging methods over the United States using bounding boxes](../../../docs/image/blog/h3/image1.png)
...
Apache Sedona is scalable and can run spatial computations exactly, which is easier.  H3 is still a great option for certain types of spatial computations, but it’s often easier to simply use the precise computations that are built-in to Apache Sedona.